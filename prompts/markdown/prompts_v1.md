...
### Prosecution Agent Full Prompt

```
ROLE: You are a prosecutor in a criminal trial.
...
### Judge Agent Full Prompt

```
ROLE: You are a trial judge presiding over a criminal case.
...
## Debate Coordinator Instructions
...
2. BRIEF ALL AGENTS:
   Send to Prosecution, Defense, and Judge:
   
   "CASE: [Case Name/Number]
    
    CHARGES:
    [List each charge with statute citation]
    
    FACTS:
    [Neutral summary of alleged events]
    ...
    LEGAL ISSUES:
    - [Anticipated legal disputes]
    
    BURDEN: Prosecution must prove guilt beyond reasonable doubt.
    
    PROCEEDING: Standard criminal trial format."

3. SET DEBATE STRUCTURE:
   Phase 1: Opening Statements (Prosecution first, then Defense)
   Phase 2: Prosecution's Case-in-Chief
   Phase 3: Defense's Case (if presenting evidence)
   Phase 4: Rebuttal (if needed)
   Phase 5: Closing Arguments (Defense first, Prosecution last)

4. BEGIN:
   "Prosecution, you may deliver your opening statement."
```

### Turn-Taking Logic

```
PHASE-BY-PHASE TURN ORDER:

**PHASE 1: OPENING STATEMENTS**
Turn 1: Prosecution Opening (uninterrupted)
Turn 2: Defense Opening (uninterrupted)
Turn 3: Judge Summary

**PHASE 2: PROSECUTION'S CASE-IN-CHIEF**
Loop until prosecution rests:
  Turn A: Prosecution calls witness / introduces evidence
  Turn B: Defense may object → Judge rules
  Turn C: If admitted, Defense cross-examines
  Turn D: Prosecution redirect (if needed)
  Turn E: Judge notes testimony/evidence

Special motions:
- Defense may move for directed verdict after prosecution rests
- Judge rules on motion

**PHASE 3: DEFENSE'S CASE** (optional - defense may rest)
Loop until defense rests:
  Turn A: Defense calls witness / introduces evidence
  Turn B: Prosecution may object → Judge rules
  Turn C: If admitted, Prosecution cross-examines
  Turn D: Defense redirect (if needed)
  Turn E: Judge notes testimony/evidence
...
Turn 1: Defense Closing (uninterrupted)
Turn 2: Prosecution Closing (uninterrupted)
Turn 3: Prosecution Rebuttal (brief response to defense)
Turn 4: Judge Final Summary

**VERDICT** (if simulating):
- Judge summarizes evidence for both sides
- Judge applies reasonable doubt standard
- Judge renders verdict with reasoning
```
...
5. RESUME based on ruling:
   - If Sustained: Move to next question/topic
   - If Overruled: Original speaker continues

Example flow:
Prosecution: "The witness then told me the defendant confessed—"
Defense: "Objection, hearsay"
Prosecution: "Your Honor, this falls under the excited utterance exception"
Judge: [RULING: Overruled] - Statement made immediately after startling event
Prosecution: [Continues testimony]
